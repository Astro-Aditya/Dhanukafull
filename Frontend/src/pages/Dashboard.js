...
function Dashboard() {
    ...
    const numberChange = async(ev) => {
      ...
      if(ev.target.value.length === 10){
        ...
        const reponse = await axios.post(`/farmer`,bod, config)
        setFarmerData(reponse.data.data)
        
        const  body = JSON.stringify({mobileNo:ev.target.value})
        const res = await axios.post(`/ticket/mobile`,body, config)
    ...
    const downloadFile = async(ev) => {
      ...
      let response = await axios.get(`/download/${fileId}`, 
      {
        responseType: 'blob'
      }, config)
   ...
   const createNewTicket = async(ev) => {
    ...
    const res = await axios.post(`/ticket`,body, config)

    // console.log(res.data.data)

    formData.append('id', res.data.data._id)

  // const res = await axios.post(`/request/upload`,formData, config)
